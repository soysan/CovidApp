...
class ViewController: UIViewController {
    ...
    func setUpGradation() {
        ...
        gradientLayer.colors = [colors.mintGreen.cgColor, colors.blueGreen.cgColor]
    ...
    func setUpContent() {
        ...
        setButton("健康管理", size: size, y: height + 190, color: colors.deepGreen, parentView: view).addTarget(self, action: #selector(goHealthCheck), for: .touchDown)
        setButton("県別状況", size: size, y: height + 240, color: colors.deepGreen, parentView: view).addTarget(self, action: #selector(goChart), for: .touchDown)
        
        setImageButton("comment", x: view.frame.size.width - 50).addTarget(self, action: #selector(goChat), for: .touchDown)
    ...
    func setButton(_ title: String, size: CGSize, y: CGFloat, color: UIColor, parentView: UIView) -> UIButton {
        ...
        let attuributedTitle = NSAttributedString(string: title, attributes: [NSAttributedString.Key.kern : 8.0])
        button.setAttributedTitle(attuributedTitle, for: .normal)
